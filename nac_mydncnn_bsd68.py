    ...
    if DATA_AUG:
        for aug in range(len(img_noisy_torch)):
            ...
            if i % 10 == 0:
                ...
                save_image("004_DnCNN_x+n1", np.clip(img_noisy_np[0], 0, 1))
                save_image("004_DnCNN_x+n1+n2", np.clip(img_noisy_noisy_np[0], 0, 1))

                if aug == 0:
                    save_image("004_DnCNN_x+n1_pred", out.detach().cpu().numpy()[0])
                    # cv2.imwrite("004_x+n1.jpg", cv2.merge(out[0] * 255))
                    
                    
            total_loss.backward()
            psrn_noisy = compare_psnr(np.clip(img_noisy_np[aug], 0, 1), out.detach().cpu().numpy()[0])
            # ssim_noisy = compare_ssim(np.clip(img_noisy_np[aug], 0, 1), out.detach().cpu().numpy()[0])
        ...
        if i % 10 == 0:
            ...
            # final_result = np.median(out_effect_np, 0)
            ...
            save_image("004_DnCNN_x_pred", final_result_reshape)
            ...
            # out_test_name = f'{i}_test'
            # save_image(out_test_name, np.clip(final_result, 0, 1), output_path=output_dir)
            if psnr_2_max==0:
                psnr_2_max = psnr_2
...
TRAIN_PLAN = [5/255.] #[5/255., 10/255., 15/255., 20/255., 25/255.]
...
output_dir = 'E:/JunXu/NAC_TPAMI/results/bsd68_g_myDnCNN_Aug_LR=0.001_Epoch=180/'
...
for index in range(0, len(files)):
    ...
    if files_name.endswith('004.png'):
        ...
        for current_sigma in range(len(TRAIN_PLAN)):
            ...
            optimize(net, OPTIMIZER, p, closure, LR, num_iter)
...
with open('l2_bsd68_g_mydncnn.pickle','wb') as f:
