...
def get_net(input_depth, NET_TYPE, pad, upsample_mode, n_channels=3, act_fun='LeakyReLU',
    ...
    if NET_TYPE == 'ResNet':
        ...
        net = UNet2(num_input_channels=input_depth, num_output_channels=3,
                   feature_scale=4, more_layers=0, concat_x=False,
                   upsample_mode=upsample_mode, pad=pad, norm_layer=nn.BatchNorm2d, need_sigmoid=True, need_bias=True)
        ...
        net = DnCNN(channels=input_depth, num_of_layers=17)
    elif NET_TYPE == 'mydncnn':
        net = MyDnCNN(channels=input_depth, num_of_layers=17)
    else:
        assert False
