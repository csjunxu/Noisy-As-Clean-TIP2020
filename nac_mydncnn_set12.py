...
def closure():
    ...
    global epoch, net_input, psnr_max, psnr_noisy_max, files_name, psnr_2_max, noisy_np
    ...
    if DATA_AUG:
        for aug in range(len(img_noisy_torch)):
            ...
            # do_i_learned_noise = net_input[aug] - out.detach().cpu().numpy()[0]
            ...
            if SAVE_DURING_TRAINING and epoch % save_every == 0:
                # output_dir
                ...
                with torch.no_grad():
                    ...
                    test_do_i_learned_noise = torch_to_np(img_noisy_np[aug]) - out_effect_np_
        ...
        if epoch % 10 == 0:
            ...
            print('%s Iteration %05d ,psnr 2: %f, psnr 2 max: %f, final ssim : %f, final ssim max: %f'
                  % (files_name, epoch, psnr_2, psnr_2_max, final_ssim, final_ssim_max))
            writer.add_scalar('final_test_psnr', psnr_2, epoch)
            writer.add_scalar('final_max_test_psnr', psnr_2_max, epoch)
        ...
        out = net(net_input)
        total_loss = mse(out, img_noisy_torch.type(dtype))

        total_loss.backward()
        psrn_noisy =  compare_psnr(np.clip(img_noisy_np, 0, 1), out.detach().cpu().numpy()[0])
        # do_i_learned_noise = out.detach().cpu().numpy()[0]
        # mse_what_tf = MSE(noisy_np, do_i_learned_noise)
        if psnr_noisy_max == 0:
            psnr_noisy_max = psrn_noisydo_i_learned_noise
        elif psnr_noisy_max < psrn_noisy:
            psnr_noisy_max = psrn_noisy
        ...
        loss_add = 0
        with torch.no_grad():
            out_effect_np = torch_to_np(net(img_noisy_torch))
            psnr_1 = compare_psnr(img_np, np.clip(out_effect_np, 0, 1))
            test_do_i_learned_noise = torch_to_np(img_noisy_np[aug]) - out_effect_np
        ...
        if epoch % 10 == 0:
            ...
            print('%s, sigma %f, Epoch %05d, psnr 2: %f, psnr 2 max: %f, final ssim : %f, final ssim max: %f, best_epoch: %d'
                  %(files_name, sigma_now*255, epoch, psnr_2, psnr_2_max, final_ssim, final_ssim_max, best_epoch))
            writer.add_scalar('final_test_psnr', psnr_2, epoch)
            writer.add_scalar('final_max_test_psnr', psnr_2_max, epoch)
            psnr_curve_max_record.append(psnr_2_max)
            ssim_curve_max_record.append(final_ssim_max)
...
output_dir = 'E:/JunXu/NAC_TPAMI/results/set12_g_myDnCNN_Aug_LR=0.001_Epoch=180/'
full_output_dir = Path(output_dir).resolve()
if not os.path.isdir(full_output_dir):
    os.mkdir(full_output_dir)
DATA_AUG = True # False  True
...
for index in range(0, len(files)):
    ...
    if files_name.endswith('.png'):
        ...
        for current_sigma in range(len(TRAIN_PLAN)):
            ...
            net = MyDnCNN(input_depth).type(dtype)
...
with open('l2_set12_g_mydncnn.pickle','wb') as f:
