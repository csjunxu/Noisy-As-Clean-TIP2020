...
np.random.seed(30)
class ISP:
    def __init__(self, curve_path='./SomeISP_operator_python/'):
    ...
    def add_PG_noise(self, img, sigma_s='RAN', sigma_c='RAN'):
        ...
        if sigma_s == 'RAN':
            ...
            sigma_s = min_log + np.random.rand(1) * (np.log([0.05]) - min_log)
            sigma_s = np.exp(sigma_s)
            self.sigma_s = sigma_s
        if sigma_c == 'RAN':
            sigma_c = min_log + np.random.rand(1) * (np.log([0.05]) - min_log)
    ...
    def cbdnet_noise_generate_srgb(self, img, sigma_s, sigma_c):
        ...
        img_mosaic_noise, sigma_s_r, sigma_c_r = self.add_PG_noise(img_mosaic, sigma_s, sigma_c)
        ...
        return img_Irgb_gt, img_Irgb, sigma_s_r, sigma_c_r
...
#     isp = ISP()
#     # path = '../testsets/DND_patches/01_noise.png'
#     path = '../figs/image_F16_512rgb.png'
#     # To node that opencv store image in BGR,
#     # When apply to color tranfer, BGR should be transfer to RGB
#     img = cv2.imread(path)
...
#
#     img_rgb_noise = isp.add_PG_noise(img_rgb)
#     show_img_tmp = np.concatenate((img, isp.RGB2BGR(img_rgb_noise),), axis=1)
#     cv2.namedWindow('Image', cv2.WINDOW_NORMAL)
#     cv2.imshow('Image', show_img_tmp)
#     cv2.waitKey(0)
#     a=1
    '''
    print('ISP test 1:')
    ...
    # noise_map = isp.cal_noise_map_srgb(noise, gt, patch_size=8)
    # # Observe the images
    # show_img = np.concatenate((img,
    #                            isp.RGB2BGR(gt),
    #                            isp.RGB2BGR(noise)
